...
|---|---|---|---|---|---|---|---|---|
| **10.4 Tiger** | **1†** | **i386** | **Yes** | **Yes** | **Yes (PVE)** | **Kext v1.9** | **Yes (HFS+)** | **Intel DVD: kext v1.7 (i386+ppc). Combo update: v1.9. Same PCI class match. dyld picks the i386 slice from the universal binary. `host_statistics64` absent — weak-imported, `vm_stat` text fallback (v2.4.1+). Serial transport `poll()`→`select()` in v2.4.2 — macOS `poll()` is kqueue-backed and the serial BSD client on 10.4 doesn't support the kqueue readiness path, so `poll()` returned `POLLNVAL` for `/dev/cu.serial1`. **v2.4.3 verifier 35/0 on MacPro2,1 — 3 freeze cycles (HFS+ root `f_fullfsync` + 1 auxiliary mount `flushed_only`, 6 special/network FS skipped) — by @vit9696 (PR #5), see [`evidence/10.4.11/`](evidence/10.4.11/), supersedes the prior v2.4.2 evidence.** **†Current runtime evidence is against v2.4.3, not the current v2.5.1+ universal artifact.** v2.5.x i386 slice uses the identical build recipe (`-Wl,-ld_classic` + legacy 10.13 SDK + `-mmacosx-version-min=10.4`) and the static verifier (`scripts/verify-legacy-slices.sh`) confirms structural equivalence on every CI build; a v2.5.1+ universal retest is welcome to close out the runtime gap. |
| **10.5 Leopard** | **1†** | **i386 only** | **Yes** | **Yes** | **Yes (PVE)** | **Kext v1.9** | **Yes (HFS+)** | **Deep verify 4/4: kext + symbols (in libc.dylib) + frameworks + PCI 0x0700. dyld picks the i386 slice from the universal binary. **v2.4.3 verifier 36/0 on MacPro2,1 — 3 freeze cycles (HFS+ root `f_fullfsync`) — by @vit9696 (PR #5), see [`evidence/10.5.8/`](evidence/10.5.8/) for the full drop including OpenCore quirks.** **†Current runtime evidence is against v2.4.3, not the current v2.5.1+ universal artifact.** v2.5.x i386 slice uses the identical build recipe as v2.4.3's; static legacy-slice verification confirms structural equivalence on every CI build. A v2.5.1+ universal retest is welcome to close out the runtime gap. |
...
| **10.11 El Capitan** | **1** | **x86_64** | **Yes** | **Yes** | **Yes (PVE)** | **Kext v3.2** | **Yes (HFS+)** | **v2.5.2 verifier 38/0 on real Xserve3,1 metal — third consecutive v2.5.x release confirmed structurally identical on this hardware (v2.5.0 → v2.5.1 → v2.5.2). Same x86_64 slice originally rebuilt under the LC_UNIXTHREAD recipe added for issue #4. See [`evidence/10.11.6/`](evidence/10.11.6/). 3 freeze cycles all clean (sync + F_FULLFSYNC on HFS+ root, 4 special FS skipped). 290/290 stress, mount-verified snapshot. Deep verify 4/4** |
