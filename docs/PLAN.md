...
| 4 | **in progress** | `scripts/verify.sh` — unified multi-transport verifier (PVE + libvirt + UTM + qga-socket), auto-detection, transport plugin architecture, PVE preflights (root / cluster locality / backup lock), auto-thaw safety trap, `host_environment` capture block, multi-cycle freeze with mount-dispatch cross-check, `--no-freeze` flag, schema 2.0 JSON appendix, shell-shim integration tests. `pve-verify.sh` deleted (no shim). Doc sweep: COMPATIBILITY/UTM/LIBVIRT/PVE/evidence. | 2026-05-23 | — |
