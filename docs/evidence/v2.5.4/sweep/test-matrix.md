...
- ~~**`guest-network-get-interfaces` intermittently times out on Tiger 10.4**~~ — **FIXED in `904783c`**. Root cause was Tiger's libc `getifaddrs()` hanging indefinitely when called from a long-running daemon (debug-logged build emitted `net-get-ifaces: entry` but never `getifaddrs done`). Same `getifaddrs()` call from a freshly-spawned SSH shell on the same Tiger VM returns < 50 ms. Fix: on Tiger 10.4 (Darwin 8.x) return an empty array immediately. Other macOS versions get the full interface list as before — verified across all 4 sweep VMs after the patch.
- ~~**`guest-network-get-route` slow on Tiger 10.4**~~ — **FIXED in `6754a36`**. Same Tiger-daemon-popen slowness pattern: `popen("netstat -rn")` takes ~11 s from the agent daemon vs ~50 ms from a fresh shell. Unlike `getifaddrs()` it does eventually return so the chardev didn't wedge, but PVE had timed out. Fix: Tiger 10.4 (Darwin 8.x) return empty array immediately. 12-command mixed sequence including BOTH network commands now passes 12/12 on Tiger.
- ~~**`--self-test-json` startup log line goes to stdout when invoked via SSH**~~ — **FALSE ALARM**. The agent's `--self-test-json` output is valid JSON when invoked correctly via `qm guest exec`. The prior observation was a `2>&1` redirect in the test invocation feeding log lines (from stderr) into the JSON parser; the agent itself is fine.
- ~~**Hook script args appear empty**~~ — **FALSE ALARM**. Re-verified with corrected test heredoc quoting: hooks DO receive `action="freeze"` / `"thaw"` as `$1`. The earlier empty `$1` observation was the writer shell expanding `$1` in the heredoc before the file was written.
