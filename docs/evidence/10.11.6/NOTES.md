...
**PVE side:** VMID 107 on PVE host `XserverLower`.
...
- **Content-based behavioural freeze check (audit finding fix)**. The `--`-decode-the-error-envelope path works correctly against real PVE on this host.
...
- This VM doesn't expose IPv4 (only link-local IPv6 fe80::). Doesn't affect the freeze path; verify.sh reported the IPv6 address correctly.
- `network-get-interfaces` showed 1 interface — single-NIC Xserve setup over the VM bridge.
- Memory: agent reports 4 of 16 blocks online → ~2 GiB used / ~8 GiB total. Block-quantized estimate; correlates with the host-side balloon-less view (PVE web UI gauge reads cgroup RSS, doesn't read the agent — see `docs/PVE.md`).
